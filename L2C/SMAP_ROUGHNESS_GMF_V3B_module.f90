...
! full roughness correction
      ...
      if (phir > -900.) then
         dew_1(1:2) = aharm(0,1:2) + aharm(1,1:2)*cos((phir)*rad) + aharm(2,1:2)*cos((2.0*phir)*rad)     ! *290K
      else
...
! sst adjustment
      ...
      if (phir > -900.)  then
         dew_2(1:2) = aharm(0,1:2) + aharm(1,1:2)*cos((phir)*rad) + aharm(2,1:2)*cos((2.0*phir)*rad)     ! *290K
      else
...
! total roughness correction
   ...
   subroutine find_demiss_rough_sstformfactor_AQV5 (irad, sst,    delta1, delta2)
      use external_files_l2c_module
! SST form factors in roughness correction
      ...
      real(4), dimension(2)                               ::  xem0, yem0, xdelta
      ...
      do ipol=1,2
         ...
         delta2(ipol) =  kappascale*xdelta(ipol)
      enddo ! ipol


      return
   end subroutine find_demiss_rough_sstformfactor_AQV5
